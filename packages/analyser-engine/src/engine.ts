...
export class AnalyserEngine {
  ...
  reset(): void {
    ...
    this.stereoBands = []
    this.waveform.l.fill(0)
    this.waveform.r.fill(0)
  }
  ...
  processPacket(packet: AudioPacket, config: AnalyserConfig): void {
    ...
    // Store oscilloscope waveform samples
    if (packet.left_wave && packet.left_wave.length > 0) {
      const n = Math.min(packet.left_wave.length, this.waveform.l.length)
      for (let i = 0; i < n; i++) {
        this.waveform.l[i] = packet.left_wave[i]
        this.waveform.r[i] = packet.right_wave?.[i] ?? packet.left_wave[i]
      }
    }
  ...
  private applyTilt(value: number, freq: number, config: AnalyserConfig): number {
    if (config.slope === 0) return value
    const octaves = Math.log2(freq / 1000)
    return value + octaves * config.slope
  }
